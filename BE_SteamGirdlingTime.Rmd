...
Mixed_model_plot = ggplot(peas_noboiled,aes(x=Time,y=Abs_Blk)) +
    ...
    geom_smooth(aes(y=peas_noboiled$fit), color = "#33CCFF", size = 1.5, 
...
```{r}
#Make plot of diameters
pdf('./figs/diam_plot.pdf')
diam_plot = ggplot(all_peas, aes(x=Diam, y=Abs_Blk, color=Trmt)) +
    geom_point()  + 
    stat_smooth(method=glm,method.args = list(family = "Gamma"), se=FALSE) +
    facet_wrap(~ Trmt, nrow = 3) + 
    theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(), 
          panel.background = element_rect(fill = "white", colour = "white"), 
          plot.background = element_rect(fill = "lightgray"), legend.position="hidden") +
    xlab("Diameter (mm)") +
    ylab("Absorbance (490 nm)") +
